...
Open [${process.env.REACT_APP_API_ENDPOINT}](${process.env.REACT_APP_API_ENDPOINT}) to view it in the browser.
...
```
Fetch API cannot load http://localhost:4000/api/todos. No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin '${process.env.REACT_APP_API_ENDPOINT}' is therefore not allowed access. If an opaque response serves your needs, set the request's mode to 'no-cors' to fetch the resource with CORS disabled.
```
